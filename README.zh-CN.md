...
10. **计划中(下一步)** —— PatchTST vs LSTM,在完整的独立无 NWP 数据集上(风电 17847 /
    光伏与负荷 18198 个窗口,3 个种子)。PatchTST 是一种把每条输入序列切成短片段、再在
    片段之间做注意力的 transformer 预测器;在与独立 LSTM 完全相同的窗口上运行,补齐
    "特征 vs 架构"这组对照,基准是已知的 LSTM 成绩(风电 702.96 / 光伏 132.43 /
    负荷 334.29)。基准刻意定为 LSTM 而**不是 Elia**:独立配置下模型不用 NWP 也不用
    TSO 预报,不可能逼近 Elia 的 185.08——拿 Elia 当基准,等于给一次受控的架构对比
    预先判了死刑
11. **计划中** —— 划分 B:整年 2024 测试划分(增量方案:划分 A 保持冻结,既有数字全部
    ...
    就没有训练数据了。划分 A 和划分 B 的数字永远不进同一张表
